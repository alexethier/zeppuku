...
`upsert_note` can assign workflow-scoped incremental `note_id` values (`1`, `2`, ...)
...
When updating a note over time, keep the same returned `note_id` and call `upsert_note`
again so history remains anchored to one logical artifact.

## Tool selection

- `upsert_note`: create/update a note and its labels
- `get_note`: read one note by `workflow_id` + `note_id` (returns content + metadata + labels)
...
- `search_notes`: return matching note identifiers with labels (`workflow_id`, `note_id`, `labels`)
...
Content source rule (strict XOR):

- Provide exactly one of:
  - `content` (inline note text), or
  - `file_path` (absolute host path to file content)

Do not pass both; do not pass neither.

## Search DSL

`search_notes` uses a JSON AST in `query`.
...
`search_notes` does not return note content in this phase. It returns IDs and labels only.

## Call patterns

### Save/update from inline content

Call `upsert_note` with:

- `content`: inline text
- no `file_path`

### Save/update from an existing file
...
Call `get_note(workflow_id, note_id)` to load prior note content and metadata.
...
- If the user asks to "find notes", default to `search_notes` first and return IDs.
- If the user asks for note content, run `search_notes` to find IDs when needed, then call `get_note` with the chosen `workflow_id` + `note_id`.
