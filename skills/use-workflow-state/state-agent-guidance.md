...
Use this as execution guidance after you read a workflow's current state. In each section, your job is to do the specified correct work needed to reach the next allowed state(s), then transition.
...
This state means deep review passed and smoke-test planning can begin.
- **Target next state:** `smoke_test_plan:drafted`.
- Create a plan for conducting smoke testing, covering all new code paths
...
This state means a smoke-test plan draft exists and needs decision.
- **Target next states:** `smoke_test_plan:accepted` or `smoke_test_plan:denied`.
...
- Persist smoke-test instructions and supporting artifacts with `ai-datastore.upsert_note(workflow_id=<workflow_id>, note_description="Smoke test plan + artifacts", name="smoke-test-plan-artifacts", labels=["artifact:smoke_test_plan","phase:testing"], content=<instructions_and_file_paths>)` (or use `file_path=<absolute_path>` when storing from an existing file).

### `smoke_test_plan:accepted`
This state means smoke-test planning is approved and instrumentation work should proceed.
...
This state means the smoke-test plan was rejected and must be revised.
- **Target next state:** `smoke_test_plan:drafted`.
...
This state means instrumentation has been added, add a new test plan that includes checking the logs from our explicit instrumentation.
...
- Confirm smoke plan plus instrumentation are fully integrated and executable.
- Execute the plan and transition when test execution is complete.
...
- Run unit and integration tests again, if they succeed, commit and push
